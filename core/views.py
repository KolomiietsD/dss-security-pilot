...
# AI-пріоритезація хостів
from .host_perceptron import (
    extract_host_features,
    generate_weak_labels,
    train_perceptron,
    load_model,
    save_model,
    predict_host_priority,
)
...
# ======================= СТАБІЛІЗАЦІЯ ПОРЯДКУ ПОДІЙ =======================

def _event_time_str(ev: Dict[str, Any]) -> str:
    """
    Дістаємо timestamp у вигляді рядка для стабільного сортування.
    Підтримуємо типові поля.
    """
...
def _event_sort_key(ev: Dict[str, Any]) -> Tuple[str, str, str, str]:
    """
    Стабільний ключ для сортування подій:
      (time_str, source, hostname, id)
    """
    if not isinstance(ev, dict):
        return ("", "", "", "")

    time_s = _event_time_str(ev)
    src = str(ev.get("source") or "")
    host = str(ev.get("hostname") or ev.get("agent_name") or "")
    ev_id = str(ev.get("id") or ev.get("event_id") or ev.get("_id") or "")

    return (time_s, src, host, ev_id)


def _sorted_events(events: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Повертає стабільно відсортований список подій.
    Це критично, бо порядок з API/агрегації може "плавати", а групування в епізоди
    і ML-аналіз тоді дають різні результати.
    """
    if not events:
        return []
...
def wazuh_view(request):
    ...
    status_counts = {"active": 0, "disconnected": 0, "never_connected": 0, "unknown": 0}
    os_counts: Dict[str, int] = {}
    last_seen_buckets = {"lt_24h": 0, "d1_7": 0, "gt_7": 0, "unknown": 0}
    ...
    charts = {
        ...
        "platforms": {"labels": list(os_counts.keys()), "data": list(os_counts.values())},
        "last_seen": {
...
def assets_data(request):
    ...
    доповнений AI-пріоритезацією хостів.

    СТАБІЛЬНІСТЬ:
    - сортуємо events перед групуванням у епізоди
    - модель тренується лише якщо її ще нема
    """
    try:
        ...
        try:
            ...
            events = _sorted_events(events)  # ✅ ключова правка
            ...
            host_features = extract_host_features(enriched)
            logger.debug("HOST FEATURES: %s", host_features)

            if host_features:
                weak_labels = generate_weak_labels(host_features)
                logger.debug("WEAK LABELS: %s", weak_labels)

                model = load_model()
                if model is None:
                    model = train_perceptron(host_features, weak_labels)
                    save_model(model)

                predictions = predict_host_priority(model, host_features)
                logger.debug("PREDICTIONS: %s", predictions)

        except Exception as ai_exc:
            logger.exception("Host AI-perceptron analysis failed: %s", ai_exc)
            predictions = {}

        for asset in unified_assets:
            hn_norm = (asset.get("key") or {}).get("hostname_norm")
            pred = predictions.get(hn_norm)

            if pred:
                asset["ai_priority_score"] = float(pred.get("priority_score", 0.0))
                asset["ai_priority_level"] = pred.get("priority_level", "low")
            else:
                asset["ai_priority_score"] = 0.0
                asset["ai_priority_level"] = "low"

        return JsonResponse(
            {"success": True, "assets": unified_assets},
            json_dumps_params={"ensure_ascii": False},
        )
    except Exception as e:
...
def asset_detections_data(request):
    ...
    if not hostname:
        return JsonResponse(
            {"success": False, "error": "Потрібен параметр hostname"},
            status=400,
            json_dumps_params={"ensure_ascii": False},
        )
    ...
    try:
        ...
        return JsonResponse(
            {"success": True, "detections": filtered},
            json_dumps_params={"ensure_ascii": False},
        )
    except Exception as e:
        logger.exception("Помилка при отриманні детекцій для hostname=%s", hostname)
        return JsonResponse(
            {"success": False, "error": str(e)},
            status=200,
            json_dumps_params={"ensure_ascii": False},
...
def wazuh_hosts_data(request):
    try:
        ...
        return JsonResponse(
            {"success": True, "hosts": agents, "agents": agents},
            json_dumps_params={"ensure_ascii": False},
        )
...
def wazuh_events_data(request):
    try:
        ...
        return JsonResponse(
            {"success": True, "events": events, "alerts": alerts, "siem_events": siem_events},
            json_dumps_params={"ensure_ascii": False},
        )
    except (WazuhAPIError, RuntimeError) as e:
...
def episodes_data(request):
    ...
    Повертає список епізодів (кластерів подій) з:
      ...
      - bert_label, bert_score, bert_sentiment
    ...
    try:
        ...
        episodes = group_events_by_time_window(events, window_seconds=window_seconds)
        analyzed = analyze_episodes(episodes, n_clusters=3)
        enriched = enrich_episodes_with_bert(analyzed)
